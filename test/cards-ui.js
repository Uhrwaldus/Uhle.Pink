...
async function main() {
  ...
  const nPacks = ev(A, `WL_CAT.packs.length`);
  const nCards = ev(A, `WL_CAT.cards.length`);
  const chips = ev(A, `document.querySelectorAll('#wl-packs [data-pack]').length`);
  ok('pack chips render', chips === nPacks, `(${chips} packs, ${nCards} cards)`);
  ...
  const nFood = ev(A, `WL_CAT.cards.filter(c => c.pack === 'food').length`);
  const pills = ev(A, `document.querySelectorAll('#wl-modal-body [data-card]').length`);
  ok('popup lists the pack', pills === nFood, `(${pills} food cards)`);
  ...
  ok('chip count drops', ev(A, `document.querySelector('#wl-packs').textContent`).includes(`${nFood - 1}/${nFood}`));
  ok('total drops', ev(A, `document.querySelector('#wl-packs').textContent`).includes(`${nCards - 1} of ${nCards}`));
  ...
  await wait(300);
  ok('all off empties the pack', ev(A, `document.querySelector('#wl-packs').textContent`).includes(`0/${nFood}`));
  ...
  ok('guest sees summary only', guestText.includes(`${nCards - nFood} of ${nCards}`) && !guestText.includes('see cards'), `("${guestText.trim()}")`);
  ...
  await wait(400);
  const nPop = ev(A, `WL_CAT.cards.filter(c => c.pack === 'pop').length`);
  ok('choices survive a reload', JSON.parse(ev(A2, `localStorage.getItem('wl.disabledCards')`)).length === nCards - nPop);
